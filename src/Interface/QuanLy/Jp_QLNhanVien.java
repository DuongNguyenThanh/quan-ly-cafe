...
public class Jp_QLNhanVien extends javax.swing.JPanel {
     ...
     */
    public static Jp_QLNhanVien tk;
    ...
    public void FillTable() {
        ArrayList<NhanVien> arrTable = new ArrayList<>();
        if (Run.tk.getPos().equalsIgnoreCase("admin")) {
            arrTable = cn.GetAdmin();
        }
        else {
            arrTable = cn.GetTaiKhoan();
        }
        DefaultTableModel tbmodel = new DefaultTableModel();

        tbmodel.addColumn("ID");
        tbmodel.addColumn("Họ");
        ...
        if (arrTable != null) {
            ...
            for (NhanVien b : arrTable) {
                sotable++;
                //Chuyển DoB sang định dạng dd/MM/yyyy trừ Admin
                String Dob = b.getDob();
                Dob = Dob.substring(0, 2) + "/" + Dob.substring(2, 4) + "/" + Dob.substring(4);
                //Đưa dữ liệu vào bảng
                tbmodel.addRow(new Object[]{b.getIdStaff(), b.getlName(), b.getfName(), Dob, b.getGender(), b.getAddress(), b.getSalary(), b.getPos()});
            }
            lblthongtin.setText(String.valueOf(sotable)+" tài khoản");
        } else {
            JOptionPane.showMessageDialog(null, "Không có tài khoản !");
        }
        JTable_TTNV.setModel(tbmodel);
        for(int i = 0; i < JTable_TTNV.getColumnCount();i++){
            Class<?> col = JTable_TTNV.getColumnClass(i);
            JTable_TTNV.setDefaultEditor(col, null);
        }        
    }
    /**
    ...
    private void bntThemActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_bntThemActionPerformed
        Dl_ThemNhanVien ban = new Dl_ThemNhanVien(Run.QlCafe, true);
        ban.setVisible(true);
    ...
    private void bntSuaActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_bntSuaActionPerformed
        int select = JTable_TTNV.getSelectedRow();
        if(select < 0){
            JOptionPane.showMessageDialog(null, "Chọn tài khoản cần sửa !");
        }else {
            String MaTable = (String)JTable_TTNV.getValueAt(select, 0);
            Dl_SuaNhanVien sua = new Dl_SuaNhanVien(Run.QlCafe, true, MaTable);
            sua.setVisible(true);
        }
    ...
    private void bntXoaActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_bntXoaActionPerformed
        int[] selectedRows = JTable_TTNV.getSelectedRows();

        if (selectedRows.length <= 0) {
            JOptionPane.showMessageDialog(null, "Chọn tài khoản cần xóa !");
        } else {
            ArrayList<String> ListMaTable = new ArrayList<String>();
            String sp = "";
            for (int i : selectedRows) {
                String ma = (String) JTable_TTNV.getValueAt(i, 0);
                ListMaTable.add(ma);
            ...
            if (qs == JOptionPane.YES_OPTION) {
                boolean xoa = cn.DeleteTaiKhoan(ListMaTable);
